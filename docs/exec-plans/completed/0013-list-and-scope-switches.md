...
- [x] Implement `docgarden list` rendering with stable sorted output, pipe escaping, fallback filename-stem names, `--config`, `--no-gitignore`, and target defaulting to `.` when no scope is selected.
...
- `list` uses the same metadata row shape as `match`, but does not apply query-term highlighting or expose color controls.
...
- [x] `docgarden list` accepts and advertises `--color` but discards it: `execute_list` binds `let _color = args.color;` and never threads it into rendering. There is no query to highlight against and no score to band, so list output gains nothing from styling. Drop the `--color` flag from `ListArgs`, the help-text assertion in `list_help_documents_output_columns_and_flags`, and the `Discoveries` note that claims `list` accepts `--color` for CLI consistency.
- [x] CLI scope-switch mutual exclusion is enforced by handwritten helpers (`match_scope`, `list_scope`, `scope_from_switches`, `list_scope_from_switches`) instead of `clap::ArgGroup`. Replacing the bool plumbing with `#[command(group(ArgGroup::new("scope").args([...]).multiple(false)))]` on `MatchArgs` and `ListArgs` deletes ~50 lines, gives parser-level error messages that name the conflicting flag, and removes the stringly-typed switch-to-`Scope` mappers in `src/scopes.rs`.
- [x] Target resolution in `execute_list` routes through nested branches that compute `cwd`, build a throwaway `vec![PathBuf::from(".")]`, and re-canonicalize it back to `cwd`. The same behavior reads as one expression: `if scope.is_some() || args.targets.is_empty() { vec![cwd] } else { canonicalize_targets(&args.targets)? }`. Simplify and drop the intermediate `invocation_targets` collection.
- [x] `ListRenderOptions` is a one-field struct (`BTreeSet<PathBuf> allow_undescribed_paths`) whose only purpose is "explicit file targets render even without a description." That requirement also forces `DocumentMetadata.absolute_path` to exist purely for the membership test. Pass the explicit-file-target slice into `print_list_rows` directly, drop the options struct, and remove `absolute_path` from `DocumentMetadata`.
- [x] `execute_match` keeps three parallel collections in lockstep: `documents`, a `Vec<String>` of `path_prefix`, and `Vec<Candidate<'_>>`. The `path_prefix` is a pure derivation of `repo_relative_path`, so storing it on `DocumentMetadata` (computed once in `load_document_metadata`) collapses the two zips and removes the standalone allocation. `list` can ignore the field.
- [x] `discover_markdown_files_for_targets` is now a one-line shim over `discover_markdown_files_for_targets_with_depth`. The `_with_depth` suffix reads as transitional scaffolding. Either rename the depth-taking function back to `discover_markdown_files_for_targets` and have the three callers pass `DirectoryDepth::Recursive` explicitly, or keep one function and drop the shim.
