...
This completed ExecPlan lives at `docs/exec-plans/completed/0003-modularize-lint-rules.md`. The active version lived in repository history while the work was in progress and while the dependent context-budget finding was reopened.
...
- [x] (2026-04-02 00:00Z) Authored the initial ExecPlan in `docs/exec-plans/completed/0003-modularize-lint-rules.md`.
- [x] (2026-04-07 00:53Z) Refreshed this plan with the context-budget dependency: modularization now must leave a clear file-level rule path for `docs/exec-plans/completed/0005-context-budget-limits.md`.
...
- [x] (2026-04-07 03:23Z) Reopened this plan alongside `docs/exec-plans/completed/0005-context-budget-limits.md` because the dependent context-budget plan recorded a blocking finding. The modularization finding itself remains non-blocking: the rule-module boundary and file-level hook still satisfy this plan.
...
- Observation: context-budget rules are a near-term downstream consumer and they are file-level rather than AST-node-local.
  Evidence: `docs/exec-plans/completed/0005-context-budget-limits.md` requires `max_tokens` and `max_lines` diagnostics for complete Markdown files at line 1, column 1. The modularization must not leave future rules with only node-level extension points.
...
5. Lint the documentation changed by this plan and the architecture update.
    ...
    cargo run -- lint docs/exec-plans/completed/0003-modularize-lint-rules.md ARCHITECTURE.md --color never
...
Validation evidence collected during implementation:
    ...
    cargo test --test path_behavior -- --nocapture
    cargo test
    cargo run -- lint docs/exec-plans/completed/0003-modularize-lint-rules.md docs/exec-plans/completed/0005-context-budget-limits.md ARCHITECTURE.md --color never
    cargo xtask validate
...
Revision note: 2026-04-07 / Planner reopened this plan from completed status because the user asked to reopen both closed plans to address recorded findings. The plan's own recorded finding remains non-blocking, but keeping it active preserves the dependency relationship while `docs/exec-plans/completed/0005-context-budget-limits.md` is fixed and re-evaluated.
