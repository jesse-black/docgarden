...
- Help text explains output columns and the score range.
...
- **Fixture strategy**: add a dedicated `tests/test-repos/discovery/` fixture so ranking assertions are isolated from lint fixtures.
...
- **Phrase bonus**: if the full normalized query appears contiguously in `name` or in the `path` basename, add 25; in `description`, add 10. Applied once per field, not per term. Phrase bonus does not use IDF — it's a flat bump for literal-substring matches.
...
- [ ] Create `src/frontmatter.rs`; move `YamlValue`, `ParsedFrontmatter`, `FrontmatterParseResult`, `parse_from_str`, and the private `parse_yaml_block` helpers from `src/lint/rules/frontmatter.rs`; keep existing unit tests co-located with the moved code.
- [ ] Add `mod frontmatter;` to `src/lib.rs`; in `src/lint/rules/frontmatter.rs`, replace the moved definitions with `pub(crate) use crate::frontmatter::{YamlValue, ParsedFrontmatter, FrontmatterParseResult, parse_from_str};` and verify lint still compiles and its inline tests still pass.
- [ ] Create `src/score.rs` implementing the scorer. Public surface:
...
- [ ] Create `src/matching.rs` with `pub fn execute_match(args: MatchArgs) -> Result<()>`:
...
- [ ] Add a dedicated fixture `tests/test-repos/discovery/` with a minimal `docgarden.toml`, a couple of docs with rich `name`/`description`, one doc with only `name`, one doc with no frontmatter (path-only discoverability), and one doc inside a gitignored subdir. Add to `tests/common/mod.rs` usage if any helpers need updating.
- [ ] Run full test + lint suite; iterate on scoring thresholds if dogfooding output against this repo's own docs and skills feels off.
- [ ] Update `docs/design-docs/frontmatter-driven-discovery-commands.md` to reflect the v1 divergences: clamped corpus-local IDF in place of uncapped IDF; `match` takes no positional targets; no fuzzy tier in v1. Follow the doc's own working-draft voice.
