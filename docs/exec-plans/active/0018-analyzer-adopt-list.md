...
- In:
  ...
  - Unit tests in `src/matching.rs` for: query `plan` highlights `Plan` (not the whole `ExecPlan`) inside an `ExecPlan` surface form; query `exec` highlights `Exec`; query `jim` highlights `Jim` in `Jim's`; query `the` does not highlight `the` in `there's`; the lowercase compound `execplan` highlights only its explicit dictionary `plan` part for query `plan`.
  - End-to-end tests in `tests/cli.rs`:
    ...
    - With `--explain` and forced color, the `Plan` half of `ExecPlan` and the `plan` dictionary part of `execplan` are wrapped in the highlight escape and the non-matching parts are not, when the query is `plan`.
...
- [x] In `src/matching.rs` (after the splitter and `is_separator` consolidation above):
  ...
  - Unit tests: query `plan` highlights `Plan` inside `ExecPlan` and not `Exec`; query `exec` highlights `Exec` inside `ExecPlan`; query `jim` highlights `Jim` inside `Jim's`; query `there` does not highlight `there` inside `there's` if Porter2 disagrees (capture and assert); query `plan` highlights only the explicit dictionary `plan` part of lowercase `execplan`.
...
- Review follow-up added `src/analyzer.rs::analyze_surface_spans`, rebased normalization and highlighting on analyzer-owned display spans, and removed matching-local possessive splitting. Focused `cargo test --lib analyzer` and `cargo test --lib matching` passed after the refactor.
- `ARCHITECTURE.md` now names `src/analyzer.rs` as the shared lexical-analysis owner for match scoring and highlighting.
- Review follow-up validation passed: `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test --lib analyzer`, `cargo test --lib matching`, `cargo test --lib score`, `cargo test --test cli match`, `cargo test`, the listed `cargo run -- match ...` smoke commands including the expected stopword-only failure, changed Markdown lint, and `cargo xtask validate`.
- User feedback clarified that an explicit compound dictionary entry is an explicit display boundary, so `execplan` now highlights only the matching dictionary part for query `plan`, matching the CamelCase `ExecPlan` behavior.
- Compound-boundary follow-up validation passed: `cargo test --lib analyzer`, `cargo test --lib matching`, `cargo test --test cli match`, changed Markdown lint, and `cargo xtask validate`.

## Review

- [x] Update `ARCHITECTURE.md` after extracting `src/analyzer.rs`: the code map still says `src/score.rs` owns shared token normalization and stopword filtering, but this branch moved the analyzer chain into `src/analyzer.rs` and reuses it from `src/score.rs` and `src/matching.rs`.
- [x] Align display highlighting with the analyzer boundary source. `src/matching.rs` currently carries display-only possessive splitting while `src/analyzer.rs` owns possessive analysis, which weakens the "one fact, one place" style goal and could drift when analyzer boundary rules change.
