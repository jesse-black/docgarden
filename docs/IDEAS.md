...
- Shared memory admission policy: explore whether a narrowly scoped cross-plan memory document, for example an operating-notes file, a cross-plan invariants file, or a deliberately constrained memory file, would help agents preserve durable repository facts that do not belong in one ExecPlan and do not fit existing top-level categories. If adopted, enforce strict admission rules so the file records only durable cross-plan facts, recurring gotchas, or stable operating invariants instead of becoming a junk drawer for temporary notes, unresolved ideas, or implementation history that should stay in ExecPlans.
