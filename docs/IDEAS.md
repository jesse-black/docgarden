...
- Minimum viable frontmatter: if metadata is stored in Markdown frontmatter, start with a small required schema such as `description`, `owner`, and `last_reviewed` rather than forcing a large universal contract. That is enough to support doc-gardening workflows, quick file triage, ownership, and freshness checks.
- Ownership checks: require `owner` to be present and non-empty for documents that are expected to stay current.
- Description checks: require `description` to be present and non-empty for documents that agents may need to triage before loading in full.
...
- Architecture-doc rules: prefer naming important files, modules, and types without brittle deep links to source lines, following the conventions documented in `ARCHITECTURE.md`.
